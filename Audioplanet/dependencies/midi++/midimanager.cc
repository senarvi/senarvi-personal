/*
    ...
    $Id: midimanager.cc,v 1.1 2007/07/04 07:26:39 seppo Exp $
*/
...
int 
Manager::remove_port (const char *name)
...
int
Manager::set_input_port (const char *tag)

{
	PortMap::iterator res;

	if ((res = ports_by_tag.find (tag)) == ports_by_tag.end()) {
		return -1;
	}
...
int
Manager::set_output_port (const char *tag)
...
Port *
Manager::port (const string &name)

{
	PortMap::iterator i;

	if ((i = ports_by_tag.find (name.c_str())) == ports_by_tag.end()) {
		return 0;
	} else {
		return (*i).second;
	}
...
int
Manager::parse_port_request (const string &str, Port::Type type)

{
	...
	/* Port specifications look like:
	   ...
	   "tagname" (optional) is the name used to refer to the
		         port. If not given, basename (devicename)
			 will be used.
	...
	*/
	...
	if (colon < str.length()) {
		...
		req->tagname = strdup (basename (req->devname));
		req->mode = O_RDWR;
	}
