...
codebase        dd      ?               ; PMCSEG linear address
...
@@initf0:                               ; no error, init protected mode
        ...
        mov eax,PMCSEG                 ; set base addx of PMCSEG
        shl eax,4
        mov [codebase],eax
...
@@vxr_initl0f0:
        ...
        mov [word ptr es:8*31h],offset int31; protected mode INT 31h
        mov [word ptr es:8*21h],offset int21; protected mode INT 21h
        mov [word ptr es:8*3],offset intr  ; protected mode INT 3
...
@@vxr_initf1:
        ...
        mov ax,cs                       ; set SELCODE descriptor (PMCSEG)
...
@@vxr_initf2:
        mov edi,[cs:codebase]           ; EDI = offset of PMCSEG from 0
        ...
        dw 0b866h,offset int15,PMCSEG   ; MOV EAX,PMCSEG:offset int15
...
@@enablea20l1:                          ; wait a single tick
        ...
        mov [word ptr bp+6],offset init_done       ; set init_done return address
...
v_pmtormsw:                             ; VCPI protected to real switch
        ...
        mov [dword ptr ds:eax+4],PMCSEG ; store real mode CS
        mov [dword ptr ds:eax+0],offset @@v_pmtormswf0     ; store real mode IP
...
xr_pmtormsw:                            ; XMS/raw protected to real switch
        ...
        push ax                         ; store AX (real mode DS)
        mov ds,[cs:selzero]             ; DS -> 0 (beginning of memory)
        mov ecx,[cs:codebase]           ; get offset of PMCSEG from 0
        ...
        and al,0feh
        mov cr0,eax
        db 0eah                         ; JMP FAR PTR PMCSEG:$+4
        dw $+4,PMCSEG                   ;  (clear prefetch que)
...
@@callbackf1:
        ...
        mov si,PMCSEG                  ; real mode target CS:IP
...
@@int310008f0:
        ...
        and [byte ptr ds:edi+ebx+6],70h ; mask offset G and high nybble of limit
...
int310009:                              ; set descriptor access rights
        ...
        and [byte ptr ds:edi+ebx+6],0fh ; mask offset high nybble access rights
...
int3103:                                ; common to 0300h, 0301h, and 0302h
        mov esi,[cs:codebase]           ; ESI = offset of PMCSEG from 0
...
@@int3103f4:
        ...
        mov [word ptr es:edi+8],PMCSEG ; return address from call
        mov [word ptr es:edi+6],offset @@int3103f1
...
int310305:                              ; get state save/restore addresses
        ...
        mov bx,PMCSEG                  ; real mode seg of same RETF
...
int310306:                              ; get raw mode switch addresses
        ...
        mov bx,PMCSEG                   ; real mode seg of rmtopm rout
...
@@int310500radjustusedf1:
        ...
        mov ebp,[cs:codebase]             ; EBP = offset of PMCSEG from 0
